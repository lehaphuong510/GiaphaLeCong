...
def get_vai_ve_list():
    try:
        df_phu = conn.read(worksheet="Data phụ")
...
def add_vai_ve_new(new_val):
    ...
    conn.update(worksheet="Data phụ", data=updated_df)
...
if submit and ten_chinh:
    ...
    data_raw.append({
        ...
        "Năm sinh - Năm mất": f"{ns_chinh} - {nm_chinh}", "Mối quan hệ với người chính": "NGƯỜI CHÍNH",
    ...
    for bd in ban_doi_list:
        if bd["ten"]:
            ...
            data_raw.append({
                ...
                "Vai vế": vv_final, "Năm sinh - Năm mất": f"{bd['ns']} - {bd['nm']}",
    ...
    conn.update(worksheet="Data Raw", data=pd.concat([df_existing, df_new], ignore_index=True))
